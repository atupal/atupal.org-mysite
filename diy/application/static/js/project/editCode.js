...
function getDocumentString() {
    ...
    request.open('POST', '/editCode/action', false);
...
function saveCode() {
    ...
    var request = new XMLHttpRequest();
    request.open('POST', '/editCode/saveCode', false);
    request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
...
function getCode() {
    var request = new XMLHttpRequest();
    request.open('GET', '/editCode/getCode', false);
    request.send(null);
...
//将用户文件夹下某个文件加载到当前代码区域
function get_file(file_name) {
    var request = new XMLHttpRequest();
    request.open('POST', '/editCode/getFile', false);
...
function deleteuser() {
    var request = new XMLHttpRequest();
    request.open('GET', '/user/deleteuser', false);
    request.send(null);
    if (request.status === 200) {
        var h = request.responseText;
        alert(h);
